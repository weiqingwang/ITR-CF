...
public class BookCluster2 {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException{
		...
		pstmt.execute();
		pstmt.close();
		pstmt=conn.prepareStatement("create table topicstarget(id int not null primary key auto_increment, topicID int not null, tag varchar(30) not null)");
		pstmt.execute();
		pstmt.close();
		
		pstmt=conn.prepareStatement("select distinct tag from tagnamestarget");
		...
		pstmt=conn.prepareStatement("insert into topicstarget(topicID,tag) values (?,?)");
		int topicID=-1;
		int last_topicID=0;
		int count=0;
		while(result1.next()){
			if(!tags_target.contains(result1.getString(1))){
				...
				pstmt.setString(2, result1.getString(1));
				pstmt.executeUpdate();
				count++;
