...
public class MovieCluster1_new {
	...
	public static void process()  throws ClassNotFoundException, SQLException{
		...
		//先建表
		pstmt=conn.prepareStatement("DROP TABLE IF EXISTS distance");
		pstmt.execute();
		pstmt.close();
		pstmt=conn.prepareStatement("create table distance(id int not null primary key auto_increment, tagID1 int not null, tagID2 int not null , distance double not null)");
		pstmt.execute();
		pstmt.close();
		
		//构造特征向量矩阵
		pstmt=conn.prepareStatement("select count(distinct tagID) from tagnames");
		...
		pstmt=conn.prepareStatement("select tagID1,tagID2,distribution from distributions");
		result=pstmt.executeQuery();
		while(result.next()){
