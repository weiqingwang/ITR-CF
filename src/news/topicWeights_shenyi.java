...
public class topicWeights_shenyi {
	public static void process() throws ClassNotFoundException, SQLException{
		...
		f.pstmt=f.conn.prepareStatement("select count(distinct tag) from tagnamesm group by topic order by topic");
		ResultSet result=f.pstmt.executeQuery();
		while(result.next())
		{
			counttags.add(result.getInt(1));
		}
		System.out.println("counttags length:"+counttags.size());
		result.close();
		f.pstmt.close();
