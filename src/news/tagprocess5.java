...
public class tagprocess5 {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException
	{
		...
		PreparedStatement pstmt=f.conn.prepareStatement("insert into tagweights(movieID,tagID,weight) values (?,?,?)");
		int count=0;
		for(int i=0;i<movieIDs.size();i++)
		{
			for(int j=0;j<tagIDs.size();j++)
			{
				...
				PreparedStatement pstmt2=f.conn.prepareStatement("select tagID from tagNames where movieID=?");
