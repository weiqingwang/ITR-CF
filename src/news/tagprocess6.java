...
public class tagprocess6 {
	...
	public static void process() throws ClassNotFoundException, SQLException
	{
		...
		PreparedStatement pstmt=f.conn.prepareStatement("delete from q");
		pstmt.execute();
		pstmt.close();
		pstmt=f.conn.prepareStatement("insert into q(tagID,movieID,weight) values (?,?,?)");
		int count=0;
