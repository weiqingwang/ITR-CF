...
public class tagprocess7target {
	...
	public static void process() throws ClassNotFoundException, SQLException
	{
		...
		PreparedStatement pstmt=f.conn.prepareStatement("delete from qqtarget");
		pstmt.execute();
		pstmt.close();
		pstmt=f.conn.prepareStatement("insert into qqtarget(tagID,movieID,weight) values (?,?,?)");
		int count=0;
		for(int i=0;i<movieIDs.size();i++)
		{
			for(int j=0;j<tagIDs.size();j++)
			{
				int n=0;
				int N=0;
				f.pstmt=f.conn.prepareStatement("select count(*) from tagnamestarget where movieID=? and tagID=?");
				...
				result.close();
				f.pstmt.close();
				f.pstmt=f.conn.prepareStatement("select count(*) from tagnamestarget where tagID=?");
