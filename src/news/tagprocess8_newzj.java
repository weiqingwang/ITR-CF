...
//计算用户对topic的评分
public class tagprocess8_newzj {
	...
	public static void process() throws ClassNotFoundException, SQLException
	{
		...
		//tag
		f.pstmt=f.conn.prepareStatement("select distinct topicID from topicstargetzj");
		result=f.pstmt.executeQuery();
		...
		f.pstmt=f.conn.prepareStatement("select topicID,movieID,weight from topicweightszj");
		result=f.pstmt.executeQuery();
		while(result.next())
		{
		...
		PreparedStatement pstmt=f.conn.prepareStatement("delete from topicratingszj");
		pstmt.execute();
		pstmt.close();
		pstmt=f.conn.prepareStatement("insert into topicratingszj(userID,topicID,rating) values (?,?,?)");
		int count=0;
		for(int i=0;i<userIDs.size();i++)
		{
