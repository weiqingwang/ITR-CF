...
public class tagprocess7_newzj {
	public static void process() throws ClassNotFoundException, SQLException{
		...
		f.pstmt=f.conn.prepareStatement("select count(distinct tag) from topicstargetzj group by topicID order by topicID");
		...
		f.pstmt=f.conn.prepareStatement("delete from topicweightszj");
		f.pstmt.execute();
		f.pstmt.close();
		f.pstmt=f.conn.prepareStatement("insert into topicweightszj(topicID,movieID,weight) values(?,?,?)");
		f.pstmt2=f.conn.prepareStatement("select sum(tagweight),topicID,movieID from temptagweightszj group by topicID,movieID");
