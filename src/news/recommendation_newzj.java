...
public class recommendation_newzj {
	...
	public static double[] process() throws ClassNotFoundException, SQLException
	{
		...
		f.pstmt=f.conn.prepareStatement("select userID,topicID,rating from topicratingszj");
		result=f.pstmt.executeQuery();
		while(result.next())
		{
			int userID=result.getInt(1);
		...
		f.pstmt=f.conn.prepareStatement("select distinct topicID from topicweightszj where movieID=?");
		int count=0;
		while(result.next())
		{
