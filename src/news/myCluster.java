...
import sourcecluster.MovieCluster1_new;
...
//	static int topicCount=110;//一共的tagID的个数是1123
	...
	{
		try {
			//MovieCluster1_new.process();
			//process();
			tempTagWeights_shenyi.process();
			topicWeights_shenyi.process();
	...
	public static void process() throws ClassNotFoundException, SQLException
	{
		...
		f.pstmt=f.conn.prepareStatement("select tagID1,tagID2,distance from distance");
		...
		f.pstmt=f.conn.prepareStatement("select userID,movieID,tagID,tag from tagnames");
		f.pstmt2=f.conn.prepareStatement("delete from tagnamesM");
		f.pstmt2.execute();
		f.pstmt2.close();
		f.pstmt2=f.conn.prepareStatement("insert into tagnamesM(userID,movieID,tagID,topic,tag) values (?,?,?,?,?)");
